    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="modal-card px-4 py-2">
          ...
          <span className="text-sm font-bold text-white">{totalMiles.toFixed(1)} miles</span>
        </div>
      </div>
          ...
          <div className="divide-y divide-[var(--modal-border)]">
            {monthEntries.map(entry => (
              <div key={entry.id} className="p-4 flex items-center justify-between">
                <div>
                  <p className="text-sm text-white">{entry.purpose}</p>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="bg-[var(--modal-card)] border-[var(--modal-border)] max-w-md">
          <DialogHeader><DialogTitle className="text-white">Log Mileage</DialogTitle></DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            <div className="space-y-2">
              <Label className="text-[var(--modal-text-muted)]">Miles</Label>
              <Input type="number" min="0" step="0.1" value={form.miles} onChange={(e) => setForm(p => ({ ...p, miles: e.target.value }))} className="bg-white/5 border-[var(--modal-border)] text-white" placeholder="12.5" />
            </div>
            <div className="space-y-2">
              <Label className="text-[var(--modal-text-muted)]">Purpose</Label>
              <Input value={form.purpose} onChange={(e) => setForm(p => ({ ...p, purpose: e.target.value }))} className="bg-white/5 border-[var(--modal-border)] text-white" placeholder="Travel to school site" />
            </div>
