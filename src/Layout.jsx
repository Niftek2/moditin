...
function AppShell({ children, currentPageName, agreed, setAgreed }) {
  ...
  return (
    ...
    >
      ...
      <main className="lg:pl-64 pt-1 flex-1" id="main-content" style={{ marginTop: isDemoMode ? "36px" : "0" }}>
        <div
...
export default function Layout({ children, currentPageName }) {
  ...
  if (["Join", "PrivacyPolicy", "DistrictPricing", "DistrictManagerDashboard"].includes(currentPageName) === false &&
    ["Join", "PrivacyPolicy", "DistrictPricing"].includes(currentPageName)) {
    return <>{children}</>;
  }
  if (["Join", "PrivacyPolicy", "DistrictPricing"].includes(currentPageName)) {
    return <>{children}</>;
  }
  ...
  return (
    <DemoProvider>
      <SubscriptionProvider>
        <SubscriptionGate>
          <AppShell currentPageName={currentPageName} agreed={agreed} setAgreed={setAgreed}>
            {children}
          </AppShell>
        </SubscriptionGate>
      </SubscriptionProvider>
